...
test00()
{
  local test_name="Authentication + QUIT"
...
test01()
{
  local test_name="PASS before USER"
  ...
  launch_test "$test_name" "$cmd2" 221
...
test02()
{
  local test_name="Command NOT ALLOWED before connect"
...
test03()
{
  local test_name="Wrong USERNAME"

  local cmd1="USER $WUSERNAME"
  local cmd2="PASS $PASS"
  local cmd3="QUIT"
...
#   local dir_name="tmpdir"
#   local dir_create="../tmpdir"

#   local cmd1="USER $USERNAME"
#   local cmd2="PASS $PASS"
#   local cmd3="CWD $dir_name"
#   local cmd4="QUIT"
...
#   launch_test "$test_name" "$cmd4" 221
#   rmdir "$dir_create"
...
test09()
{
  ...
  local snd_param="fazogjorg"

  local cmd1="USER $USERNAME"
  local cmd2="PASS $PASS"
  local cmd3="CDUP"
  local cmd4="CDUP $snd_param"
  local cmd5="QUIT"
  ...
  launch_test "$test_name" "$cmd2" 230
  launch_test "$test_name" "$cmd3" 250
  launch_test "$test_name" "$cmd4" 500
  launch_test "$test_name" "$cmd5" 221
